# plugins/gemini_melody_generator.py
import random
import pretty_midi
import numpy as np
...
class GeminiMelodyGenerator(PluginBase):
    """
    MIDI generator plugin that creates melodies using Google's Gemini AI
    Uses Gemini's language models to interpret natural language instructions and generate music
    """
    
    def __init__(self):
        super().__init__()
        self.name = "Gemini Melody Generator"
        self.description = "Generates melodies using Google's Gemini language models"
        ...
        self.parameters = {
            ...
            "gemini_model": {
                ...
                "description": "Gemini model ID (editable for latest models)"
            },
            "temperature": {
                "type": "float",
                "min": 0.0,
    ...
    def _load_api_keys(self):
        ...
        try:
            cache_path = os.path.expanduser("~/.gemini_melody_generator_api_key.json")
...
"""
    ...
    def _query_gemini(self, system_prompt: str, user_prompt: str, api_key: str, model: str, temperature: float, max_tokens: int) -> str:
        """
        Query Gemini API using the official SDK
        ...
        try:
            ...
            try:
                ...
                raise ImportError("Google Generative AI SDK not installed. Please install it with: pip install google-generative-ai")
    ...
    def _prompt_with_progress_indicator(self, system_prompt: str, user_prompt: str, api_key: str, model: str, temperature: float, max_tokens: int) -> str:
        """
        Prompt Gemini with a progress indicator
        ...
        try:
            ...
            print(f"\rGenerating with Gemini {progress_chars[i]} ", end="")
            while thread.is_alive():
                i = (i + 1) % len(progress_chars)
                elapsed = time.time() - start_time
                # This would be replaced with your UI's progress indicator
                print(f"\rGenerating with Gemini {progress_chars[i]} {elapsed:.1f}s", end="")
                time.sleep(0.1)
    ...
    def generate(self, existing_notes=None, **kwargs):
        ...
        gemini_model = kwargs.get("gemini_model", "gemini-1.5-pro")
        ...
        if not api_key and "gemini_api_key" in self.api_key_cache:
            api_key = self.api_key_cache["gemini_api_key"]
            
        # Save key to cache if provided
        if api_key:
            self.api_key_cache["gemini_api_key"] = api_key
            self._save_api_keys()
        
        # Check for API key
        if not api_key:
            raise ValueError("No Gemini API key provided. Please enter an API key in the parameters.")
        ...
        if not note_sequence:
            print("Could not parse any notes from the Gemini response.")
            print("Response:", response[:500], "..." if len(response) > 500 else "")
            raise ValueError("Could not extract notes from the Gemini response.")
